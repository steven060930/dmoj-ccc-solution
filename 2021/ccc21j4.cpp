...
int main() {
	...
	string s; read(s);
	ll cnt_l = 0, cnt_m = 0, cnt_s = 0;
	for (int i = 0; i < sz(s); i++) {
		if (s[i] == 'L') cnt_l++;
		if (s[i] == 'M') cnt_m++;
		if (s[i] == 'S') cnt_s++;
	}
	string tmp1 = s.substr(0, cnt_l), tmp2 = s.substr(cnt_l, cnt_m), tmp3 = s.substr(cnt_l + cnt_m, cnt_s);
	print(count(tmp3, 'M') + count(tmp3, 'L') + max(count(tmp1, 'M'), count(tmp2, 'L')));
}
